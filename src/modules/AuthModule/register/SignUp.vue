...
<template>
  <div class="flex flex-col w-full min-h-screen items-center py-4 px-7 gap-3">
    ...
    <p class="text-black text-[1.25rem] font-bold tracking-[.0125rem]">Sign Up</p>
    <form class="flex flex-col w-full gap-5">
      <InputAuth label="Email" placeholder="Masukkan Email" :LeftIcon="MessageIcon"/>
      <InputAuth label="Password" placeholder="Masukkan Password" :LeftIcon="LockIcon" :rightIcon="true"/>
      <InputAuth label="Konfirmasi Password" placeholder="Masukkan Password" :LeftIcon="LockIcon" :rightIcon="true"/> 
      <button
        type="submit"
