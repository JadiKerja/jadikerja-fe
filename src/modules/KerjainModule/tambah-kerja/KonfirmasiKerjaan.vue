...
import PencilIcon from '@/assets/images/PencilIcon.vue'
...
<template>
  <div class="h-screen flex flex-col items-center w-full">
    <div class="bg-[rgb(214,39,39)] flex flex-col items-center relative w-full pb-[4.5rem] pt-[3rem] px-7 gap-6 overflow-hidden">
      <TransparentCircle
    ...
    <div class="w-full rounded-[1.875rem] mt-[-2rem] flex flex-col px-7 pb-7 pt-5 gap-5 z-20 bg-[#F8FAFF] h-fit overflow-y-auto custom-scrollbar-hidden">
      <ProgressIcon :pageNumber="3" />
      <div class="flex flex-col gap-1">
        <p class="text-black text-[1.25rem] font-semibold">
          Konfirmasi
        </p>
        <p class="text-black text-[0.75rem]">
          Mohon periksa kembali pekerjaan kamu
        </p>
      </div>
      <div class="flex flex-col gap-1 w-full">
        <div class="flex flex-row justify-between">
          <p class="text-black text-[0.875rem] font-semibold">Informasi Pribadi</p>
          <button @click="emit('back')" class="bg-[#F4A91E] rounded-[0.625rem] p-[0.3125rem] flex flex-row gap-[1px] font-semibold text-[0.75rem] text-white">
            <PencilIcon />
            <p>Edit</p>
          </button>
        </div>
        <div class="w-full p-[0.9375rem] flex flex-col gap-[0.625rem] bg-[#EEF0F3] rounded-[0.9375rem]">
          <div class="flex flex-col gap-1">
            <p class="text-[0.625rem] font-medium text-[#AEACAC]">
              Nama Lengkap
            </p>
      ...
      <div class="flex flex-col gap-1 w-full">
        <div class="flex flex-row justify-between">
          <p class="text-black text-[0.875rem] font-semibold">Informasi Pekerjaan</p>
          <button @click="emit('editIsiKerjaan')" class="bg-[#F4A91E] rounded-[0.625rem] p-[0.3125rem] flex flex-row gap-[1px] font-semibold text-[0.75rem] text-white">
            <PencilIcon />
            <p>Edit</p>
          </button>
        </div>
      ...
      <button @click="emit('next')" class="w-full py-[0.8125rem] text-white bg-[#D62727] text-center font-semibold rounded-[0.9375rem]">
        Konfirmasi
      </button>
    </div>
  </div>
